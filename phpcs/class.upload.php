...
class upload
{
	...
	public $cl_message_01 = "Le fichier téléchargé exc&egrave;de la taille de upload_max_filesize, configurée dans le php.ini.";
	// message d'erreur si le fichier téléchargé excède la taille de MAX_FILE_SIZE, qui a été spécifiée dans le formulaire HTML
	public $cl_message_02 = "Le fichier dépasse la limite autorisée dans le formulaire HTML.";
	// message d'erreur si le fichier n'a été que partiellement téléchargé
	public $cl_message_03 = "L'envoi du fichier a été interrompu pendant le transfert.";
	// message d'erreur si aucun fichier n'a été téléchargé
	public $cl_message_04 = "Aucun fichier n'a été téléchargé.";
	...
	public $cl_message_06 = "Echec de l'écriture du fichier sur le serveur.";
	...
	public $cl_message_12 = "Le nom du fichier existe déj&agrave;.";
	// message d'erreur si il y a eu une erreur inconnue !!!
	...
	public $cl_message_15 = "Le répertoire ou doit &ecirc;tre envoyé le fichier n'existe pas.";
	...
	public function __construct($repUpload="./", $clfichier)
	{
		...
		$this->cl_fichier = basename($_FILES[$this->cl_file]['name']);		
		$extFichier = strrchr($_FILES[$this->cl_file]['name'], '.');
	...
	{	
		...
		else
		{
			...
			echo $this->cl_dossier;
			return false;
		}
		
		
		// vérification des erreurs de chargement de fichiers
		if ($_FILES[$this->cl_file]['error'])
		{
			switch ($_FILES[$this->cl_file]['error'])
			{
		...
		else if ($_FILES[$this->cl_file]['size'] >= $this->cl_taille_maxi)
		{
		...
		else if(move_uploaded_file($_FILES[$this->cl_file]['tmp_name'], $this->cl_dossier.$this->cl_new_name))
		{
			return true;			
		}
	...
	public function cGetSizeFile($type=0, $ponc=",")
	{
		...
		$valeur = $_FILES[$this->cl_file]['size'];
		$retour = "";
	...
	{
		return $_FILES[$this->cl_file]['tmp_name'];	
	}
	...
	{
		return $_FILES[$this->cl_file]['type'];	
	}	
}
?>